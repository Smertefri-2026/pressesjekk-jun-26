...
function BuyPackageContent() {
  ...
  async function startCheckout() {
    ...
    if (error || !session) {
      ...
      const next = `/checkout${currentQuery || `?plan=${selectedPlan.id}`}`;
      ...
      <section className="mx-auto max-w-4xl px-4 py-10 sm:px-6 sm:py-14 lg:px-8">
        ...
        <div className="mt-8">
          ...
          <h1 className="mt-4 text-4xl font-black tracking-tight text-slate-950 sm:text-5xl">
            Fullfør kjøpet trygt.
          </h1>

          <p className="mt-5 text-lg leading-8 text-slate-700">
            Kjøpet knyttes til Min Side, slik at rapportpakken kan lagres, brukes
            på en sak og bygges videre med dokumentasjon senere.
          </p>
        </div>
        ...
        {!selectedPlan ? (
          <section className="mt-8 rounded-3xl border border-slate-200 bg-white p-6 shadow-sm sm:p-8">
            ...
            <Link
              href="/priser"
              className="mt-6 inline-flex rounded-xl bg-red-500 px-5 py-3 font-black text-slate-950 hover:bg-red-500"
            >
              Se priser
            </Link>
          </section>
          ...
          <section className="mt-8 rounded-3xl border border-slate-200 bg-white p-6 shadow-sm sm:p-8">
            <p className="inline-flex rounded-full bg-red-100 px-3 py-1 text-xs font-black uppercase tracking-[0.15em] text-red-800">
            ...
            {selectedAmount > 0 ? (
              <div className="mt-5 rounded-2xl bg-slate-950 p-5 text-white">
                ...
                {discountAmount > 0 ? (
                  <div className="mt-3 flex justify-between gap-4 text-sm font-bold text-orange-200">
            ...
            <div className="mt-7 rounded-2xl border border-red-200 bg-red-50 p-4 text-sm leading-7 text-slate-700">
              For at rapportpakken skal kunne lagres trygt, må kjøpet knyttes til en
              konto på Min Side. Er du ikke innlogget, sender vi deg først til
              innlogging eller konto-opprettelse og deretter videre til betaling.
            </div>
            ...
            <button
              ...
              disabled={isLoading}
              className="mt-7 w-full rounded-xl bg-red-500 px-5 py-4 text-center font-black text-slate-950 hover:bg-red-500 disabled:cursor-not-allowed disabled:opacity-60"
            >
              {isLoading ? "Sender deg til betaling ..." : "Fortsett til betaling"}
            </button>
          </section>
        )}
      </section>
